"""
Deep reinforcement learning with DEP as part of the network input, sampling kappa bandit-style, and dep exploration
"""
...
actor.load_state_dict(torch.load('deep_dep_results/imitation.pth'))
...
for e in range(num_episodes):
    ...
    actor.dep_controller.kappa = kappa
    total_reward = 0
    total_actor_loss = 0
    total_critic_loss = 0
    if e % 500 == 0:
    ...
    kappa_sampler.update(kappa, total_reward)
...
# Save metrics 
data = np.array([episode_reward, episode_actor_loss, episode_critic_loss, kappas])
cols=['reward', 'actor_loss', 'critic_loss', 'kappas']
